...
def action(request,domain=None):
    ...
    try:
        ...
        try:
            ...
            # check status Waf
            WafSetting = settingManager.Waf(provision.provision_name)
            onOff = WafSetting.perform(action='status')
            # get log waf
            listLog = []
            if onOff == 'on':
...
def changeStatus(request,pro_id=None):
    ...
    try:
        ...
        if request.POST:
            ...
            WafSetting = settingManager.Waf(provision.provision_name)
            onOff = WafSetting.perform(action=list_status[status])
            if onOff:
                result['status'] = 1
                result['msg'] = 'You change status WAF success!'
            else:
                result['msg'] = 'Can not turn on/off WAF. Please try again!'
    except KeyError:
        return HttpResponseRedirect('/login')
    except ObjectDoesNotExist:
